...
function setCurrentPod(podId, endpoint) {
  ...
  if (!podId || !(endpoint && endpoint.ip)) {
    currentPodLabel.textContent = '—';
    currentPodLabel.title = 'Not connected';
    return;
  }
  const label = `${endpoint.ip}:${endpoint.port}`;
  currentPodLabel.textContent = label;
  currentPodLabel.title = `GPU node · ${label}`;
}
...
function applyConfigToUI(config) {
  if (!config) return;
  if (cfgBackendUrl) { cfgBackendUrl.value = config.backendUrl || ''; cfgBackendUrl.dispatchEvent(new Event('input')); }
...
function updateLicenseUI() {
  ...
  if (btnLicenseLogin) btnLicenseLogin.style.display = licenseLoggedIn ? 'none' : 'block';
  if (btnLicenseLogout) btnLicenseLogout.style.display = licenseLoggedIn ? 'block' : 'none';
...
async function runLaunchSequence() {
  ...
  // Huge wordmark whose letters light up one by one over ~10s.
  const letters = launchOverlay.querySelectorAll('.launch-wordmark span');
  const lightMs = 8400;
  const step = letters.length ? lightMs / letters.length : 0;
  ...
  await sleep(1200);             // hold fully lit, then dissolve
...
async function startStreaming(ip, port) {
  ...
  hsDone(_hsResolve);
  const _hsCam = hsStep('opening camera');
...
function stopStreaming() {
  document.body.dataset.status = 'idle';   // return to the connect console
  gpuIp = null;
  gpuPort = null;
  _lastGpuIp = null;
  _lastGpuPort = null;
...
// Selected source face, cached so it can be applied to the GPU node once we're
// connected (BYO: the node only exists after Connect, so we can't POST earlier).
let _faceBuffer = null;
let _faceName = '';

async function applyFaceToGpu() {
  if (!_faceBuffer) return;
  setLog('Applying face...');
  try {
    await window.chimera.uploadFace(_faceBuffer, _faceName);
    setLog('Face applied.');
  } catch (err) {
...
faceInput.addEventListener('change', async () => {
  ...
  try {
    _faceBuffer = await file.arrayBuffer();
    _faceName = file.name;
  } catch (_) {
    setLog('Could not read the selected image.');
    return;
  }

  // If we're already streaming, push it now; otherwise it's applied on Connect.
  if (document.body.dataset.status === 'live') {
    await applyFaceToGpu();
  } else {
    setLog('Face ready — it will be applied when you connect.');
  }
});

// Transport "Change face" (visible while live) re-opens the picker.
const btnFaceLive = document.getElementById('btn-face-live');
if (btnFaceLive) btnFaceLive.addEventListener('click', () => faceInput.click());
...
btnStart.addEventListener('click', async () => {
  ...
  document.body.dataset.status = 'arming';
  setStatus('Arming...', 'loading');
  setLog('Resolving GPU node...');
  setLoading(true);
  showLoadingState('Connecting...', 'Resolving your GPU node and opening the stream.');

  // Persist the GPU node URL from the console so the main process can resolve it.
  try {
    await window.chimera.saveAppConfig({
      backendUrl: cfgBackendUrl.value,
      obsPort: (cfgObsPort && cfgObsPort.value) || 7891,
    });
  } catch (_) {}

  hsReset();
  _hsResolve = hsStep('resolving ' + (cfgBackendUrl.value.trim() || 'GPU node'));
  ...
  try {
    ...
    document.body.dataset.status = 'idle';   // failed — return to the console
    stopBeeping();
    setStatus('Error', 'error');
    maybeShowKeyContactPopup(err.message);
...
// ── Phase B: connect-console wiring (dropzone, drag-drop, live ws:// validation) ──
(function () {
  const dz = document.getElementById('face-dropzone');
  const fi = document.getElementById('face-input');
  const fp = document.getElementById('face-preview');
  const urlInput = document.getElementById('cfg-backend-url');
  const urlCheck = document.getElementById('gpu-url-check');

  // Click anywhere on the dropzone (except the Browse button) opens the file picker.
  if (dz && fi) {
    dz.addEventListener('click', (e) => {
      if (e.target.id !== 'btn-upload') fi.click();
    });
  ...
  // Mark the dropzone once a face image is set (drives the thumbnail vs empty state).
  if (fp && dz) {
    const mark = () => { if (fp.getAttribute('src')) dz.classList.add('has-face'); };
    fp.addEventListener('load', mark);
    try { new MutationObserver(mark).observe(fp, { attributes: true, attributeFilter: ['src'] }); } catch (_) {}
    mark();
  }

  // Live ws:// validation glyph.
  if (urlInput && urlCheck) {
    const validate = () => {
      const v = urlInput.value.trim();
      if (!v) { urlCheck.dataset.state = 'empty'; urlCheck.textContent = '○'; return; }
      const ok = /^((wss?|https?):\/\/)?[^\s:/]+(:\d{2,5})?$/.test(v);
      urlCheck.dataset.state = ok ? 'ok' : 'bad';
      urlCheck.textContent = ok ? '✓' : '✕';
    };
...
// ── Phase C: connect handshake terminal ──
const _hsLog = document.getElementById('hs-log');
let _hsResolve = null;   // bridges the connect handler → startStreaming
function hsReset() { if (_hsLog) _hsLog.innerHTML = ''; }
function hsStep(text) {
  if (!_hsLog) return null;
  const el = document.createElement('div');
  el.className = 'hs-line';
  el.dataset.state = 'run';
  el.innerHTML = '<span class="hs-mark">◇</span><span class="hs-text"></span>';
  el.querySelector('.hs-text').textContent = text;
  _hsLog.appendChild(el);
  return el;
}
function hsDone(el) {
  if (!el) return;
  el.dataset.state = 'ok';
  const m = el.querySelector('.hs-mark');
  if (m) m.textContent = '◆';
}
function hsLive(text) {
  if (!_hsLog) return;
  const el = document.createElement('div');
  el.className = 'hs-line hs-live';
  el.dataset.state = 'ok';
  el.innerHTML = '<span class="hs-mark">●</span><span class="hs-text"></span>';
  el.querySelector('.hs-text').textContent = text;
  _hsLog.appendChild(el);
...
// ── Phase D: auto-hide chrome while live & idle (the feed owns the screen) ──
(function () {
  const IDLE_MS = 3000;
  let idleTimer = null;
  function wake() {
    ...
    if (document.body.dataset.status === 'live') {
      idleTimer = setTimeout(() => {
        if (document.body.dataset.status === 'live') document.body.classList.add('chrome-idle');
      }, IDLE_MS);
    }
  }
  ['mousemove', 'mousedown', 'keydown', 'wheel', 'touchstart'].forEach((ev) =>
    window.addEventListener(ev, wake, { passive: true }));
  // Re-arm the idle countdown whenever we enter/leave live.
  try { new MutationObserver(wake).observe(document.body, { attributes: true, attributeFilter: ['data-status'] }); } catch (_) {}
})();

// ── Phase E: command palette (⌘K) + settings drawer (⌘,) ──
(function () {
  ...
  const isLive = () => body.dataset.status === 'live';

  const openDrawer  = () => { body.classList.remove('palette-open'); body.classList.add('drawer-open'); };
  const closeDrawer = () => body.classList.remove('drawer-open');
  const toggleDrawer = () => body.classList.contains('drawer-open') ? closeDrawer() : openDrawer();

  const ACTIONS = [
    { label: 'Connect',                     hint: '',   show: () => !isLive(), run: () => click('btn-start') },
    { label: 'Disconnect',                  hint: '',   show: () =>  isLive(), run: () => click('btn-stop') },
    { label: 'Start / stop recording',      hint: 'rec',           run: () => click('btn-record') },
    { label: 'Choose face image',           hint: '',              run: () => click('btn-upload') },
    { label: 'Copy OBS Browser Source URL', hint: '',              run: () => {
        const u = document.getElementById('obs-url');
        if (u && navigator.clipboard) navigator.clipboard.writeText(u.textContent.trim()).catch(() => {}); } },
    { label: 'Open setup guide',            hint: '',              run: () => click('btn-open-tutorial') },
    { label: 'Download DroidCam (virtual camera)', hint: '',        run: () => { try { window.chimera.openDriversFolder().catch(() => {}); } catch (_) {} } },
  ];
  ...
  function render() {
    ...
    filtered = ACTIONS.filter((a) => (!a.show || a.show()) && a.label.toLowerCase().includes(q));
    sel = 0;
    if (!filtered.length) { palList.innerHTML = '<div class="pal-empty">No commands</div>'; return; }
    palList.innerHTML = filtered.map((a, i) =>
      `<div class="pal-item${i === 0 ? ' sel' : ''}" data-i="${i}"><span></span><span class="pal-hint">${a.hint || ''}</span></div>`).join('');
    // set labels as textContent (avoid injecting label text as HTML)
    palList.querySelectorAll('.pal-item').forEach((el, i) => { el.firstChild.textContent = filtered[i].label; });
  }
  function move(d) {
    ...
    if (items[sel]) { items[sel].classList.add('sel'); items[sel].scrollIntoView({ block: 'nearest' }); }
  }
  function runSel() {
    const a = filtered[sel];
    closePalette();
    if (a) setTimeout(() => a.run(), 0);
  }
  function openPalette() { closeDrawer(); body.classList.add('palette-open'); palInput.value = ''; render(); setTimeout(() => palInput.focus(), 0); }
  function closePalette() { body.classList.remove('palette-open'); }
  function togglePalette() { body.classList.contains('palette-open') ? closePalette() : openPalette(); }
  ...
  if (palList) palList.addEventListener('click', (e) => {
    ...
    sel = Number(it.dataset.i); runSel();
  });
  const dC = document.getElementById('drawer-close');
  const dS = document.getElementById('drawer-scrim');
  const pS = document.getElementById('palette-scrim');
  if (dC) dC.addEventListener('click', closeDrawer);
  if (dS) dS.addEventListener('click', closeDrawer);
  if (pS) pS.addEventListener('click', closePalette);
  const railK = document.getElementById('rail-cmdk');
  if (railK) railK.addEventListener('click', openPalette);

  window.addEventListener('keydown', (e) => {
    const mod = e.metaKey || e.ctrlKey;
    if (mod && (e.key === 'k' || e.key === 'K')) { e.preventDefault(); togglePalette(); }
    else if (mod && e.key === ',') { e.preventDefault(); toggleDrawer(); }
